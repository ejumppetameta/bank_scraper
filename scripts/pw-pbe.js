...
////////
///////////////////////////////////////////////////////////////
async function processSavingCurrentAccountRows(page, accountRows) {
  for (let i = 0; i < accountRows.length; i++) {
    ...
    if (row) {
      ...
      if (accountLink) {
        ...
        await Promise.race([
          page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 10000 }).catch(() => console.log('⚠️ Navigation timeout, continuing...')),
          accountLink.click(),
        ])

        console.log(`✅ Checking transactions for account ${accountNumber}...`)

        // ✅ Ensure "View Transaction History" button exists
        try {
          await page.waitForSelector('text=View Transaction History', { timeout: 5000 })
          await Promise.all([
          ...
          console.error('❌ Failed to navigate to transaction history page.')
          continue
        }

        console.log('✅ Now on the transaction history page!')

        // ✅ Check if transactions table is already loaded
        const isTableAlreadyLoaded = await page.$('table#table-column-toggle tbody tr')
        if (isTableAlreadyLoaded) {
          console.log('✅ Transactions table is already loaded. Skipping reload.')
        }
        else {
          console.log('🔄 Waiting for transactions table to load...')
          await page.waitForSelector('table#table-column-toggle tbody tr', { timeout: 10000 })
        }
        ...
        // ✅ Now, you can access the extracted values:
        const fullName = accountDetails.accountName
        const accountNo = accountDetails.accountNumber
        const accountType = await page.$eval('#TABLE_2 tr:nth-child(3) td:nth-child(3)', el => el.textContent.trim())
        const openingBalance = Number.parseFloat(accountDetails.openingBalance) || 0
        const closingBalance = Number.parseFloat(accountDetails.closingBalance) || null
        ...
        const months = await getLatestMonths(page)

        console.log('✅ Selected Months:', months)

        for (const month of months) {
          console.log(`🔄 Processing transactions for month: ${month}`)

          // ✅ Select the month, but only trigger a reload if necessary
          const currentMonthValue = await page.evaluate(() => {
            return document.querySelector('select[name="mState"]').value
          })

          if (currentMonthValue !== month) {
            console.log(`🔄 Changing month from ${currentMonthValue} to ${month}`)
            await page.selectOption('select[name="mState"]', month)
            await page.waitForTimeout(2000) // Small wait for table refresh
          }
          else {
            console.log(`✅ Month ${month} is already selected. Skipping reload.`)
          }

          const statementMonth = await getStatementMonth(page)

          const transactions = await scrapeHtmlTransactions(page)

          console.log(`✅ Extracted ${transactions.length} transactions for ${month}`)

          if (transactions.length > 0) {
            const openingBalance = Number.parseFloat(transactions[0]?.balance.replace(/,/g, '') || '0')
            ...
            try {
              ...
              const bankStatementId = response.data.id

              console.log('✅ Bank statement stored with ID:', bankStatementId)

              // ✅ Ensure each transaction includes `bank_statement_id`
              const updatedTransactions = transactions.map(txn => {
                // ✅ Remove all non-numeric characters except dot (.)
                const cleanNumber = value => {
                  return Number.parseFloat(value.replace(/[^\d.-]/g, '').trim()) || 0
                }

                // ✅ Convert debit & credit to numbers, default to 0 if empty
                const debitAmount = cleanNumber(txn.debit)
                const creditAmount = cleanNumber(txn.credit)

                // ✅ Ensure amount is always present and formatted correctly
                const amount = (debitAmount > 0 ? debitAmount : creditAmount > 0 ? creditAmount : 0).toFixed(2)

                // ✅ Ensure balance is a valid decimal
                const balance = cleanNumber(txn.balance).toFixed(2)

                // ✅ Fix transaction_date formatting (DD-MM-YYYY → YYYY-MM-DD)
                let formattedDate = null
                if (txn.date) {
                  const cleanDate = txn.date.trim() // Trim spaces

                  // ✅ Ensure it matches DD-MM-YYYY format before converting
                  if (/^\d{2}-\d{2}-\d{4}$/.test(cleanDate)) {
                    ...
                    console.error(`❌ Invalid date format found: ${txn.date}`)
                    formattedDate = null // Set to null if invalid
                  }
                }

                return {
                  ...txn,
                  bank_statement_id: bankStatementId, // ✅ Ensure bank_statement_id is included
                  amount, // ✅ Ensure amount is always a valid decimal
                  balance, // ✅ Ensure balance is a valid decimal
                  transaction_date: formattedDate, // ✅ Rename `date` to `transaction_date`
                  transaction_type: debitAmount > 0 ? 'DR' : creditAmount > 0 ? 'CR' : null,
                }
              })

              // Send transactions to API
              await storeTransactionsViaAPI(updatedTransactions)
            }
            catch (error) {
              console.error('❌ API Error (Bank Statement Store):', error.response?.data || error.message)
            }
          }
        }
      }
      else {
        console.log(`❌ Inactive or no <a> found in row: ${await row.innerText()}`)
      }
    }
    else {
      console.log('❌ No valid row found')
...
async function scrapeHtmlTransactions(page) {
  const transactions = []
  ...
  while (hasNextPage) {
    try {
      // ✅ Ensure table is available before scraping
      await page.waitForSelector('table#table-column-toggle tbody tr', { timeout: 10000 })

      console.log(`📄 Scraping transactions - Page ${pageCounter}`)

      const pageTransactions = await page.evaluate(() => {
        const rows = document.querySelectorAll('table#table-column-toggle tbody tr')
        ...
        return Array.from(rows).map(row => {
          ...
          return {
            ...
            debit: cells[2]?.innerText.trim() || '0',
            credit: cells[3]?.innerText.trim() || '0',
            balance: cells[4]?.innerText.trim() || '0',
          }
        })
      })

      if (pageTransactions.length === 0)
        console.warn('⚠️ No transactions found on this page.')

      transactions.push(...pageTransactions)

      // ✅ Detect if the same transactions are being reloaded
      if (transactions.length === lastTransactionCount) {
        console.log('⚠️ No new transactions detected. Stopping pagination.')
        break
      }
      ...
      const totalPages = await page.evaluate(() => {
        const pagesElement = document.querySelector('#t1pages')

        return pagesElement ? Number.parseInt(pagesElement.innerText.trim(), 10) : 1
      })

      if (totalPages === 1) {
        console.log('✅ Only one page exists. Stopping pagination.')
        break
      }

      // ✅ Check if "Next" button exists, is visible, and is enabled
      hasNextPage = await page.evaluate(() => {
        const nextButton = document.querySelector('.table-page\\:next')

        return nextButton && nextButton.style.display !== 'none' && !nextButton.classList.contains('disabled')
      })

      if (hasNextPage) {
        console.log(`➡️ Clicking "Next" to fetch more transactions... (Page ${pageCounter})`)

        try {
          await Promise.race([
            page.click('.table-page\\:next'),
            page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 10000 }).catch(() => console.log('⚠️ Next page load timeout')),
          ])
          await page.waitForTimeout(2000)
        }
        catch (error) {
          console.error('❌ Failed to click "Next" button:', error)
          break // Stop loop if "Next" fails
        }

        pageCounter++
      }
      else {
        console.log('✅ No more pages detected. Stopping pagination.')
      }
    }
    catch (error) {
      console.error('❌ Error during transaction scraping:', error)
      break // Stop loop if an error occurs
    }
  }

  console.log(`✅ Completed transaction scraping: ${transactions.length} transactions found.`)

  return transactions
}

async function storeTransactionsViaAPI(transactions) {
  try {
    const response = await axios.post(
      'http://localhost/api/store-bank-transactions',
      { transactions }, // ✅ Send transactions with `bank_statement_id`
    )
...
async function getAccounts(page) {
  return await page.evaluate(() => {
    const selectBox = document.querySelector('select[name="LIST_FROM_ACC_NO"]')

    return Array.from(selectBox.options)
      .map(option => option.value)
      .filter(value => value !== '' && value !== 'all')
  })
}

async function getLatestMonths(page) {
  return await page.evaluate(month_length => { // ✅ Pass it as an argument
    const monthSelect = document.querySelector('select[name="mState"]')
    if (!monthSelect)
      return []

    return Array.from(monthSelect.options)
      .map(option => option.value.trim()) // ✅ Ensure values are trimmed
      .filter(value => value !== '') // ✅ Remove empty values
      .slice(0, month_length) // ✅ Limit to the latest N months
  }, month_length) // ✅ Pass variable as an argument
}

async function getStatementMonth(page) {
  return await page.evaluate(() => {
    const selectedOption = document.querySelector('select[name="mState"] option:checked')
    if (!selectedOption)
      return null

    const monthValue = selectedOption.value // Example: "202503"
...
async function loginWithPlaywright(accessId, password) {
  ...
  try {
    ...
    const links = await page.$$('a[href*="MethodName=formAccBal"]')

    for (const link of links) {
      const text = await link.innerText()
      if (text.includes('Savings / Current Account')) {
        console.log(`✅ Clicking: ${text}`)
        await link.click()
        break // Stop after clicking the right link
      }
    }

    await logAndScreenshot(page, '✅ Successfully navigated to the account page.')

    // 1. Wait for the table to be visible
    await page.waitForSelector('#table-column-toggle')

    // 2. Get all table rows within tbody
    const accountRows3 = await page.$$('#table-column-toggle tbody tr')

    // console.log('🔍 For future checking purpose, DO NOT DELETE below code')
    await logAndScreenshot(page, '🔍 For future checking purpose, DO NOT DELETE below code')

    // console.log(`🔍 Found ${accountRows} of account`)
    await logAndScreenshot(page, `🔍 Found ${accountRows3} of account`)

    // 3. Loop for get how many accounts in the table
    if (accountRows3.length > 0) {
      // console.log(`✅ Found ${accountRows.length} account rows. Processing...`)
      await logAndScreenshot(page, `✅ Found ${accountRows3.length} account rows. Processing...`)
      await processSavingCurrentAccountRows(page, accountRows3)
    }
    else {
      // console.error('❌ No account rows found!')
      await logAndScreenshot(page, '❌ No account rows found!')
