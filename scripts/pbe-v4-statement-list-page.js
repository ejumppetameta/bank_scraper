...
async function downloadStatementsForAllAccounts(page) {
  ...
  for (const account of accountOptions) {
    ...
    await downloadLatestStatements(page)
 ...
 */
async function downloadLatestStatements(page) {
  console.log('🔍 Retrieving available statements...')
  ...
  for (const stmt of latestStatements) {
    ...
    // Extract parameters from the onclick function
    ...
    if (match) {
      ...
      console.log('✅ Statement download initiated.')

      // Wait for a few seconds before proceeding to the next one
